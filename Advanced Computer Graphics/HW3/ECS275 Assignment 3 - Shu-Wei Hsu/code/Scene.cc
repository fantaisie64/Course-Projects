...
void Scene::newRender()
{
    ...
    for(int i=0;i<yres;i++){
        ...
        for(int j=0;j<xres;j++){
            ...
            // regular ray tracing with area light and antialiasing
            /*int arealightSampleTimes = 4;
            for(int k = 0; k<arealightSampleTimes; k++){
            ...
            // path tracing with area light and antialiasing
            // direct illumination
            Color directResult(0,0,0);
            double directWeight = 0.0;
            int directSampleTimes = 64;
            for(int k = 0; k<directSampleTimes; k++){
                double offset_x = (drand48() - 0.5) * pixelWidth;
                double offset_y = (drand48() - 0.5) * pixelHeight;
                camera->makeRay(ray, context, x+offset_x, y+offset_y);
                directResult += newTraceRay(context, object, ray, atten, 1, 0);
                directWeight += 1.0;
            }
            directResult /= directWeight;
            
            // indirect illumination
            Color indirectResult(0,0,0);
            double indirectWeight = 0.0;
            int indirectSampleTimes = 200;
            for(int k = 0; k<indirectSampleTimes; k++){
                double offset_x = (drand48() - 0.5) * pixelWidth;
                double offset_y = (drand48() - 0.5) * pixelHeight;
                camera->makeRay(ray, context, x+offset_x, y+offset_y);
                indirectResult += pathTraceRay(context, object, ray, atten, 1, 0);
                indirectWeight += 1.0;
            }
            indirectResult /= indirectWeight;
            
            result = (directResult + indirectResult);
            /*if(tempResult.r() >= tempResult.g() && tempResult.r() >= tempResult.b()){
                result = Color(1.0,tempResult.g()/tempResult.r(),tempResult.b()/tempResult.r());
            }else if(tempResult.g() >= tempResult.r() && tempResult.g() >= tempResult.b()){
                result = Color(tempResult.r()/tempResult.g(), 1.0,tempResult.b()/tempResult.g());
            }else if(tempResult.b() >= tempResult.r() && tempResult.b() >= tempResult.g()){
                result = Color(tempResult.r()/tempResult.b(),tempResult.g()/tempResult.b(), 1.0);
            }*/
    ...
    if(depth >= maxRayDepth || atten.maxComponent() < minAttenuation){
        ...
        if(hit.getPrimitive()){
            ...
            for(int i = 0 ; i < lights.size(); i++){
                if(lights[i]->hitLight(ray)){
                    //Vector lightDirection;
                    //Color lightColor;
                    //lights[i]->getLight(lightColor, lightDirection, context, Point(0.0, 0.0, 0.0));
...
Color Scene::pathTraceRay(const RenderContext& context, const Object* obj, const Ray& ray, const Color& atten, int depth, double time) const
{
    Color result(0, 0, 0);
    ...
    if(hit.getPrimitive()){
        ...
        for(int i=0;i<lights.size();i++){
            ...
            if(!shadowHit.getPrimitive()){
                ...
                result += matl->getColor() * diffuseColor;
            }
        }*/
        
        // indirect illumination
        Vector randomDirection = Vector(drand48()-0.5, drand48()-0.5, drand48()-0.5 );
        randomDirection.normalize();
        while(Dot(randomDirection, normal) <= 0 ){
            randomDirection = Vector(drand48()-0.5, drand48()-0.5, drand48()-0.5 );
            randomDirection.normalize();
        }
        Ray reflectedRay = Ray(intersectionPoint + randomDirection * 0.0001f, randomDirection);
        
        result += indirectTraceRay(context, object, reflectedRay, atten, depth+1, time)*Dot(randomDirection, normal);
        
        // use indirect color as light
        /*Vector lightDirection = Vector(drand48()-0.5, drand48()-0.5, drand48()-0.5 );
        lightDirection.normalize();
        while(Dot(lightDirection, normal) <= 0 ){
            lightDirection = Vector(drand48()-0.5, drand48()-0.5, drand48()-0.5 );
            lightDirection.normalize();
        }
        Ray reflectedRay = Ray(intersectionPoint + lightDirection * 0.0001f, lightDirection);
        Color lightColor = indirectTraceRay(context, object, reflectedRay, atten, depth+1, time);
        ...
        result += matl->getColor() * (diffuseColor + specularColor);*/
        // end use indirect color as light
...
Color Scene::indirectTraceRay(const RenderContext& context, const Object* obj, const Ray& ray, const Color& atten, int depth, double time) const
{
    Color result(0, 0, 0);
    // Russian Roulette
    double russianRouletteP = 0.8;
    if(depth >= maxRayDepth || atten.maxComponent() < 1.e-5 || drand48()>russianRouletteP)
        return result;
    ...
    if(hit.getPrimitive()){
        ...
        /*double normalizationFactor = 2.0;
        double ratio = (hit.minT() != 0) ? (normalizationFactor/hit.minT()) : 1.0;
        ratio = (ratio > 1.0) ? 1.0 : ratio;*/
        
        Color reflectionColor(0,0,0);
        //float reflectionFactor = 0.8;
        //if (reflectionFactor > 0.0f){
            //Vector reflected = 2.0f * Dot((-ray.direction()), normal) * normal - (-ray.direction());
            Vector randomDirection = Vector(drand48()-0.5, drand48()-0.5, drand48()-0.5 );
            randomDirection.normalize();
            ...
            Ray reflectedRay = Ray(intersectionPoint+ randomDirection * 0.0001f, randomDirection); // avoid reflecting to itself
            double dotProduct = Dot(randomDirection, normal);
            reflectionColor = indirectTraceRay(context, object, reflectedRay, atten*dotProduct, depth+1, time) * dotProduct;
        //}
