  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "eye" ) )
        ...
        throwParseException( "Expected `eye', `lookat', `up', `hfov', `aperture', `focal'  or }." );
    }
  return new PinholeCamera( eye, lookat, up, hfov, aperture, focal );
}
    ...
    if ( peek( Token::left_brace ) )
        for ( ; ; )
        {
            if ( peek( "position" ) )
                ...
                throwParseException( "Expected `position', `color', `length', `width' or }." );
        }
    return new AreaLight( position, normal, length, width, color );
}
...
Material *Parser::parseLambertianMaterial()
{
  ...
  return new LambertianMaterial( color, Kd, Ka, Ks, Kr );
}
  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "material" ) )
        ...
        throwParseException( "Expected `material', `point', `normal', `speed' or }." );
    }
  return new Plane( material, normal, point, speed );
}
    ...
    if ( peek( Token::left_brace ) )
        for ( ; ; )
        {
            if ( peek( "material" ) )
                ...
                throwParseException( "Expected `material', `point', `speed' or }." );
        }
    return new Polygon( material, points, speed );
}
  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "material" ) )
        ...
        throwParseException( "Expected `material', `center', `radius', `speed' or }." );
    }
  return new Sphere( material, center, radius, speed );
}

Object *Parser::parseCubeObject()
{
    Material *material = default_material;
    Point center( 0.0, 0.0, 0.0 );
    Vector speed( 0.0, 0.0, 0.0 );
    ...
    if ( peek( Token::left_brace ) )
        for ( ; ; )
        {
            if ( peek( "material" ) )
                ...
                speed = parseVector();
            else if ( peek( "sideLength" ) )
                sideLength = parseReal();
            else if ( peek( Token::right_brace ) )
                break;
            else
                throwParseException( "Expected `material', `center', `sideLength', `speed' or }." );
        }
    return new Cube( material, center, sideLength, speed );
}
...
Parser::Parser(
  ...
  : input( input ),
    line_number( 1 ),
    column_number( 0 ),
    default_material( new LambertianMaterial( Color( 1.0, 1.0, 1.0 ), 0.6, 0.3, 0.4, 0.2 ) )
{
  readNextToken();
}
  ...
  string &filename )
{
  ...
  for ( ; ; ) {
    ...
    else if ( peek( "antialiasingSampleTimes" ) )
      scene->setAntialiasingSampleTimes( parseInteger() );
    else if ( peek( "depthOfFieldSampleTimes" ) )
      scene->setDepthOfFieldSampleTimes( parseInteger() );
    else if ( peek( "motionBlurSampleTimes" ) )
      scene->setMotionBlurSampleTimes( parseInteger() );
    else if ( peek( "light" ) )
